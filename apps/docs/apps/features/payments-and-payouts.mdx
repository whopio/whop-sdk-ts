...
import { makeWebhookValidator, type ReceiptData } from "@whop/api";
...
async function handlePaymentSucceededWebhook(data: ReceiptData) {
...
<Warning>
  Make sure to create a webhook and set your `WHOP_WEBHOOK_SECRET` environment variable. You can find it in your [dashboard](https://whop.com/dashboard/developer/) app settings under the Webhooks section.
</Warning>
