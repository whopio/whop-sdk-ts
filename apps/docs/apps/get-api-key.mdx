---
title: Get an API key
description: All requests to Whop APIs are managed using a secure API key.
---

1. Go to https://whop.com/dashboard/developer/.
2. Click the **Create app** button.
3. Give your app a name and click **Save**.
4. Copy the API key from the `Environment variables` section and use it in your code.

<video autoPlay loop muted controls className="rounded-xl border border-white/10" src="/how-to-videos/copy-environment-variables.mp4" title="Video demonstrating how to copy environment variables from the Whop dashboard" aria-label="Video demonstrating how to copy environment variables from the Whop dashboard" />

### Company API (deprecated) -- V2 and V5

Legacy. Old system for when you want to automate your own creator account or sync payments/crm data to your internal systems.
