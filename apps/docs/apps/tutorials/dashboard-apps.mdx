...
4. For each permission, provide a justification of why your app needs it. Then also choose wether it is required (or optional). You can toggle the little switch at the top of each card. When complete, _CLICK SAVE_
...
<Info>
  ...
  you then update you requested permissions, they will see the same prompt to
  re-grant then again when using your app.
</Info>

6. Now you can start making API requests for data on the companies that you have authorize. (We are adding more endpoints) But right now you can list the receipts (payments) for a company:
