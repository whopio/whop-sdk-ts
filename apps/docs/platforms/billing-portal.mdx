...
Platforms can expose billing management in two ways:

- **Deep link**: render a button in your UI that opens the membership’s `manage_url` in a new tab.
- **Embed pattern (pseudo)**: create a page in your platform that proxies Whop data and links (recommended for tighter UX without iframing the portal).

```tsx
// Deep link example
export function ManageSubscription({ manageUrl }: { manageUrl: string }) {
  return (
...
```ts
// Server pseudo-code to fetch memberships and surface manage links
import { Whop } from "@whop/api";

export async function listBuyerMemberships(userId: string) {
  const whop = new Whop({ apiKey: process.env.WHOP_API_KEY });
  const whopUserId = await mapToWhopUserId(userId);
  const { data: memberships } = await whop.company.memberships.list({
    user_id: whopUserId,
  });
...
<Note>
  Avoid iframing the portal directly; the URL requires an authenticated Whop
  session. Deep-link or show the `manage_url` as an action from your own list of
  memberships.
</Note>
