...
4. Add event handlers for status changes:
...
You can also connect to the websocket from your client side frontend code running in the whop iframe.
Use this to add real-time features to your frontend app, and receive messages you sent from your server. 
...
1. Create the websocket client:
	...
	const websocket = whopApi.websocketClient({
		joinExperience: "exp_XXXX", // optional, you can join a specfic experience channel (ie, the one you are currently viewing).
		joinCustom: "some_custom_channel", // optional, you can join a custom channel.
	});
	```

2. Add event handlers for messages, similar to the server websocket:
...
3. Handle connection status changes (same as server API):
