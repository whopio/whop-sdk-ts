...
2. Create a new project and copy your database password
...
Replace your password with `[YOUR-PASSWORD]` and your project reference with `[YOUR-PROJECT-REF]`.
...
## 4. Create the experience helper
...
## 6. Create Required Components

First, create the `ImageUploader` component in `components/image-uploader.tsx`. This component handles image upload and generation:
...
Then, create the `ExperiencePrompt` component in `components/experience-prompt.tsx`:
...
## 6. Implement the experience page

Go to `/experiences/[experienceId]/page.tsx` and replace the contents with:
...
## 7. Create the API route to generate images
